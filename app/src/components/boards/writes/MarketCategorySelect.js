...
const MarketCategorySelect = ({ navigation }) => {
  ...
  return (
    <>
      ...
      <ItemContainer onPress={() => setIsElectronics(!isElectronics)}>
        <Image source={require("../../../icons/market/electronics.png")} />
        <ItemTitle>{t.print("Electronic")}</ItemTitle>
        <ItemIcon onPress={() => setIsElectronics(!isElectronics)}>
          <AntDesign name='down' size={30} color={theme.label} />
        </ItemIcon>
      </ItemContainer>
      {isElectronics ? (
          ...
          <ItemContainer
            onPress={() =>
              navigation.navigate("MarketWritePage", {
                category: {
                  no: 12,
                  name: `${t.print("Book")}/${t.print("NotebookWithKnowhow")}`,
                },
              })
            }
          >
