...
const RedText = styled.Text`
  color: ${({ theme }) => theme.buttonUnfilledTitles};
  position: absolute;
  right: 0px;
  top: 0px;
`;
...
const PostInfo = ({
  ...
  const transformisBargaining = () => {
    if (product.isBargaining) return <GreenText>흥정가능</GreenText>;
    else return <RedText>흥정불가</RedText>;
  };
  return (
    <Container>
      ...
      <PostInfoBottomContainer>
        ...
        <Price>{`${conversionPrice()}${t.print("Won")}`}</Price>
      </PostInfoBottomContainer>
      <>{_selectStatus()}</>
    </Container>
  );
